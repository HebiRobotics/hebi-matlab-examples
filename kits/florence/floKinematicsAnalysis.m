function [] = floKinematicsAnalysis( log, kin, legIndex )
%%
...
for i = 1:length(log.time)
    tempFrame = kin.leg{leg}.getFK('endeffector',log.positionCmd(i,legIndex{leg}));
    cmdXYZ(i,:) = tempFrame(1:3,4);
    tempFrame = kin.leg{leg}.getFK('endeffector',log.position(i,legIndex{leg}));
...
for i = 1:length(log.time)
    J_cmd(:,:,i) = kin.leg{leg}.getJacobian( 'endeffector', log.positionCmd(i,legIndex{leg}) );
    J_fbk(:,:,i) = kin.leg{leg}.getJacobian( 'endeffector', log.position(i,legIndex{leg}) );
    
    cmdVel_6DoF(i,:) = J_fbk(:,:,i) * log.velocityCmd(i,legIndex{leg})';
    fbkVel_6DoF(i,:) = J_fbk(:,:,i) * log.velocity(i,legIndex{leg})';
end
