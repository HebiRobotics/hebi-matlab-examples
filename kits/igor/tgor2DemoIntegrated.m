...
function tgor2DemoIntegrated( cam_module )
...
while true
    try
        ...
        robotGroup = HebiLookup.newGroupFromNames('T-gor',moduleNames);
        break;
...
gains = HebiUtils.loadGains([localDir '/gains/tgorLegGains.xml']);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Robot Kinematics %
    %%%%%%%%%%%%%%%%%%%%
    [legKin, armKin, chassisKin] = makeTgorKinematics(numLegs, numArms);
    chassisMass = getBodyMasses(chassisKin);
    %     chassisCoM = [0; 0; .10 + .3];  % XYZ center of mass (m)
    chassisCoM = [0; 0; .10 + .05];  % XYZ center of mass (m)
