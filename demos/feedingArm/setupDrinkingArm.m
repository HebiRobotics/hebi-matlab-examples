function [ group, kin, gravityVec ] = setupDrinkingArm()
...
gains = HebiUtils.loadGains('defaultDrinkingGains.xml');
group.send('gains',gains);

% End Effector Kinematics
xyz_endEff = [.030 0 .01];   % Bottle opener
rot_endEff = R_y(pi/2);
...
kin.addBody('X5-9');
kin.addBody('X5-Link', 'extension', .325, 'twist', pi, 'mass', .300);
kin.addBody('X5-9');
kin.addBody('X5-Link', 'extension', .325, 'twist', pi, 'mass', .300);
kin.addBody('X5-1');
